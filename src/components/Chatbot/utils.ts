...
export const getCustomStylesHeaderColor = (config: IConfig) => {
  if (config.customStyles) {
    return config.customStyles.headercolor;
  }
  return {};
};
...
export const getavatarIcon = (config: IConfig) => {
  if (config.avatarIcon) {
    return config.avatarIcon;
  }
  return {};
};
